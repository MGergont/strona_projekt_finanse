...
function generowanieRaport($wynik){
...
function statystyki($zakres){
    ...
    $wynik=mysqli_query(polonczenie_mysql(),'SELECT SUM(kwota) AS suma_kwota, round((-SUM(kwota)*100)/(SELECT SUM(kwota)*(-1) as suma_koszt FROM dane WHERE id_kategoria != 11 AND id_users = '.$user.' AND data_time > "'.$data2.'" AND data_time < "'.$data1.'"), 2) as procent, nazwa_kat FROM (SELECT kwota, kategoria.nazwa_kat FROM dane INNER JOIN kategoria ON dane.id_kategoria=kategoria.id_kategoria WHERE dane.id_kategoria!=11 AND dane.id_users = '.$user.' AND data_time > "'.$data2.'" AND data_time < "'.$data1.'") AS test GROUP BY nazwa_kat;') or die("Problemy z odczytem danych!");

    // SELECT SUM(kwota)*(-1) as suma_koszt FROM dane WHERE id_kategoria != 11 AND id_users = 2 AND data_time > '2023-05-15 00:00:00' AND data_time < '2023-06-01 00:00:00';

// SELECT SUM(kwota) AS suma_kwota, round((-SUM(kwota)*100)/16793, 2) as procent, nazwa_kat FROM (SELECT kwota, kategoria.nazwa_kat FROM dane INNER JOIN kategoria ON dane.id_kategoria=kategoria.id_kategoria WHERE dane.id_kategoria!=11 AND dane.id_users = 2 AND data_time > '2023-05-15 00:00:00' AND data_time < '2023-06-01 00:00:00') AS test GROUP BY nazwa_kat;

// SELECT SUM(kwota) AS suma_kwota, round((-SUM(kwota)*100)/(SELECT SUM(kwota)*(-1) as suma_koszt FROM dane WHERE id_kategoria != 11 AND id_users = 2 AND data_time > '2023-05-15 00:00:00' AND data_time < '2023-06-01 00:00:00'), 2) as procent, nazwa_kat FROM (SELECT kwota, kategoria.nazwa_kat FROM dane INNER JOIN kategoria ON dane.id_kategoria=kategoria.id_kategoria WHERE dane.id_kategoria!=11 AND dane.id_users = 2 AND data_time > '2023-05-15 00:00:00' AND data_time < '2023-06-01 00:00:00') AS test GROUP BY nazwa_kat;
